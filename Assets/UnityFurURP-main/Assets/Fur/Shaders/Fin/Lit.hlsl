...
float4 frag(Varyings input) : SV_Target
{
    ...
    if (input.finUv.x >= 0.0 && furColor.a < _AlphaCutout)
        discard;
    ...
    float3 pattern = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, input.finUv).xyz;
    
    surfaceData.albedo = lerp(_Red, _G, pattern.y).xyz;
